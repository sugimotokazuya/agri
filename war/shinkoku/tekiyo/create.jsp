...
<div class="title">摘要登録 - ${f:h(kamoku.name)}</div>
...
<input type="hidden" name="key" value="${f:h(kamoku.key)}"/>
<input type="hidden" name="version" value="${f:h(kamoku.version)}"/>
<input class="w50" type="button" value="戻る" onclick="window.location.href='./?key=${f:h(kamoku.key)}&version=${kamoku.version}';"/>
</form>
</body>
</html>
