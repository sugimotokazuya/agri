...
<div class="contentBox">
	<div class="content">
		...
		<form action="${f:url('insert')}" method="post">
			<input type="hidden" name="torihikisaki" value="${f:h(torihikisaki.key)}"/>
			<table>
				<tr>
					<th>取引先</th>
					<td>${f:h(torihikisaki.name)} 様</td>
				</tr>
				<tr>
					<th>請求金額</th>
					<td><fmt:formatNumber value="${seikyuKingaku}" pattern="###,###,##0"/> 円</td>
				</tr>
				<tr>
					<th>発行日</th>
					<td>
			...
			<table>
				<tr>
					...
					<th style="width:450px;">
						...
						<div style="width:20%;float:left;text-align:center;">数量</div>
							...
							<table style="border:0px;width:100%;background-color:transparent;">
								<c:forEach var="e2" items="${e.recList}" varStatus="s">
									<tr>
										...
										<td style="border:0px;width:20%;background-color:transparent;<c:if test="${!s.last}">border-bottom:dashed 1px gray;</c:if>">　<fmt:formatNumber value="${e2.suryo}" pattern="###,###,##0"/>【${f:h(e2.shukkaKeitaiRef.model.hoso)}】</td>
			...
			<hr/>
			<button type="submit">請求データ作成</button>
