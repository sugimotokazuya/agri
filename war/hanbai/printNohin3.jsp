...
<div style="text-align:center;font-size:xx-large;">納品書 兼 請求書</div>

<table class="noborder" style="width:100%;" border="0">
<tr>
	<td style="width:70%;">
		<div style="font-size:large;">
			<c:if test="${s.okurisaki==''}">${f:h(s.torihikisakiRef.model.name)}</c:if>
			<c:if test="${s.okurisaki!=''}">${f:h(s.okurisaki)}</c:if>
			 様
		</div>
		<div style="margin-top:20px;">ご注文ありがとうございました。<br/>下記の通りご請求申し上げます。</div>
		<div style="margin-top:20px;">${f:h(s.torihikisakiRef.model.biko)}</div>

		<div style="font-size:x-large;margin-top:18px;">
			ご請求金額：<fmt:formatNumber value="${seikyuKingaku}" pattern="###,###,##0"/>円
		</div>
	</td>
	<td style="width:30%;font-size:x-small;">
		...
		<div style="margin-top:20px;">
			発行日：<fmt:formatDate value="${date}" pattern="yyyy年MM月dd日"/>
		</div>
		<div>
			支払期限：<fmt:formatDate value="${limitDate}" pattern="yyyy年MM月dd日"/>
		</div>
		</div>
		</div>
	</td>
