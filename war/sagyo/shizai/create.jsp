...
<form action="${f:url('insert')}" method="post">
	...
	<div class="contentBox">
		<div class="content">
			<h1>使用資材登録 - ${f:h(sagyoItem.name)}</h1>
			<h2>使用資材名</h2>
			...
			<br/><span class="${f:errorClass('riyu', 'err')}">${f:h(errors.riyu)}</span><br/>
			
			
			<hr/>
			<button type="submit">使用資材登録</button>
			<button type="button" onclick="window.location.href='./?key=${f:h(sagyoItem.key)}&version=${sagyoItem.version}';">使用資材管理へ戻る</button>
