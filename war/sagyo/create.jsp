...
function save() {
	var useGazo1 = document.getElementById("useGazo1");
	var elements = useGazo1.childNodes;
	for(i=0; i < elements.length; ++i) {
		ele = elements[i];
		if(ele.className=='gazo') {
			if($('#gazoHidden').val() != '') $('#gazoHidden').val($('#gazoHidden').val() + ','); 
			$('#gazoHidden').val($('#gazoHidden').val() + ele.id);
		}
	}
	return true;
}
...
<div class="contentBox">
	<div class="content">
		<h1>
			作業登録[<span class="strong">${f:h(sagyoItem.name)}</span>]
			-${f:h(sakuduke.hatakeRef.model.name)} / ${f:h(sakuduke.name)}
			<span class="small">
				[担当者：${f:h(sakuduke.tantoRef.model.name)}] 
				[開始日：<fmt:formatDate value="${sakuduke.startDate}" pattern="yyyy-MM-dd"/>]
			</span>
		</h1>
		<form action="${f:url('insert')}" method="post">
			<input type="hidden" ${f:hidden("sakudukeKey")}/>
			...
			<hr/>
			<button type="submit" onclick="return save();">作業登録</button>
			<button type="submit" onclick="$('#continue').val('1');return save();">連続して登録</button>
			<input type="hidden" id="continue" name="continue" value="0"/>
			<input type="hidden" name="gazoHidden" id="gazoHidden" />
